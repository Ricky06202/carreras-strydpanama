...
export default function AdminDashboard() {
  ...
  const loadCategories = (raceId: string) => {
    fetch(`${API_BASE}/api/categories/${raceId}`)
      .then(r => {
  ...
  const loadDistances = (raceId: string) => {
    fetch(`${API_BASE}/api/admin/distances?raceId=${raceId}`)
      .then(r => {
        if (!r.ok) return r.json().then(e => Promise.reject(e));
        return r.json();
      })
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/admin/races`)
      .then(r => {
        if (!r.ok) return r.json().then(e => Promise.reject(e));
        return r.json();
      })
  ...
  const handleCreateDistance = async (data: Partial<Distance>) => {
    if (!selectedRace) return;
    const res = await fetch(`${API_BASE}/api/admin/distances`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...data, raceId: selectedRace.id })
    });
  ...
  const handleUpdateDistance = async (id: string, data: Partial<Distance>) => {
    const res = await fetch(`${API_BASE}/api/admin/distances/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
  ...
  const handleDeleteDistance = async (id: string) => {
    const res = await fetch(`${API_BASE}/api/admin/distances/${id}`, { method: 'DELETE' });
    if (res.ok && selectedRace) loadDistances(selectedRace.id);
  };

  const handleUpdateParticipant = async (id: string, data: Partial<Participant>) => {
    const res = await fetch(`${API_BASE}/api/admin/participant/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
  ...
  const handleDeleteParticipant = async (id: string) => {
    const res = await fetch(`${API_BASE}/api/admin/participant/${id}`, { method: 'DELETE' });
    if (res.ok && selectedRace) {
      fetch(`${API_BASE}/api/admin/race/${selectedRace.id}`)
        .then(r => r.json())
        .then(d => { if (d.participants) setParticipants(d.participants); })
